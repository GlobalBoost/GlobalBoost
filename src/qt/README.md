This directory contains the GlobalBoostQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### globalboostgui.(h/cpp)

Represents the main window of the GlobalBoost UI.
...
ClientModel is used by the main application `globalboostgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/globalboost/globalboost/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a globalboost:... link from a browser).
...
* UI elements like GlobalBoostAmountField, which inherit from QWidget.
* `globalbooststrings.cpp`: automatically generated
* `globalboostunits.(h/cpp)`: BSTY / mBSTY / etc handling
* `callback.h`
...
* don't change `local/globalboost_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "globalboost-qt" under "Run", which is where you can also add command line arguments)
